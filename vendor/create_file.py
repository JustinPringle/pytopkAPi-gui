...
def strahler_stream_order(start_arc_id, start_up_node,
    ...
    if len(arcs_per_node[start_up_node]) == 1:
            ...
            if arc_id != start_arc_id:
                ...
                if up_node != start_up_node:
                    ...
                    upstream_orders[arc_id] = strahler_stream_order(arc_id,
                                                                  ...
                                                                  segments)
            # print(upstream_orders[arc_id])
            # segment_ids[upstream_orders[arc_id]]+=1
        ...
        if max_order_count > 1:
            ...
            # segment_ids[upstream_orders[start_arc_id]]-=1
        # print(up_node,down_node)    
        # segments[start_arc_id] = segment_ids[upstream_orders[start_arc_id]]
...
def strahler_to_channel_manning(cell_labels, channel_network, cell_down):
    ...
    channel_network = np.asarray(channel_network, dtype=np.int64)
    cell_down = np.asarray(cell_down, dtype=np.int64)

    # compute strahler order
    nodes = cell_labels[np.where(channel_network == 1)[0]]
    
    #this can cause problems if you have slight errors in your flow direction (and cell down) map
    edges = []
    ...
    #strahler for channel network only
    # for k in range(len(nodes)-1):
    #     # if (channel_network[k] == 1) and (cell_down[k] >= 0):
    #     edges.append((nodes[k], nodes[k+1]))
...
def cell_connectivity(flowdir, mask, source='GRASS'):
    ...
    cell_id = np.ones(mask.shape, dtype=np.int64)*int_min
    cell_id[mask == 1] = np.arange(ncells)

    cell_down = np.ones(ncells, dtype=np.int64)*int_min
...
def channel_properties(cell_labels, channel_network, X, Y, cell_down, dem):
    ...
    Xc = np.zeros(cell_labels.shape, dtype=np.float64)
    tan_beta_channel = np.zeros(cell_labels.shape, dtype=np.float64)
